...
class TestConverterExpressions
{
    ...
    {
        val function = functionOf("foo", valueType = Builtin.INT32.id).also { symbols.register(it) }
    ...
    {
        val boolVar = variableOf("var_bool", Builtin.BOOL.id).also { symbols.register(it) }
        val int32Var = variableOf("var_int32", Builtin.INT32.id).also { symbols.register(it) }
        val int64Var = variableOf("var_int64", Builtin.INT64.id).also { symbols.register(it) }
